# ha-bridge Color Replacement Reference

This guide provides a complete reference for using ha-bridge color replacement patterns with the Matterbridge HTTP plugin.

## Overview

The plugin now supports **ha-bridge color replacements**, enabling advanced RGB/HSB color handling for devices with complex requirements. These replacements work in both URLs and parameter values for all color-related endpoints.

Color values are derived from the device's current hue, saturation, and brightness states. The plugin automatically converts between HSV (Hue-Saturation-Value) and RGB color spaces as needed.
...
- [Summary](HA_BRIDGE_SUMMARY.md) - For quick overview
